...
function startTimer() {
    timer = setInterval(() => {
        ...
        time = (real_time / 100).toFixed(2);
        playerTime.innerHTML = time;

        if (time === '0.00' || checkClear()) {
            clearInterval(timer);
            stopGame();
        }
...
function showCardDeck() {
    ...
    let showCardPromise = new Promise((resolve, reject) => {
        let showCardTimer = setInterval(() => {
            ...
            if(cnt-1 >= 0) {
                cardBack[cnt-1].style.transform = "rotateY(0deg)";
                cardFront[cnt-1].style.transform = "rotateY(-180deg)";
            }
    ...
    showCardPromise.then(() => {
        ...
        setTimeout(hideCardDeck, 500);
    })
}

// 전체 카드 숨기는 함수
function hideCardDeck() {
    // showCardDeck함수에서 카드를 보여줄때 이전 카드를 숨기는 방식으로 변경함.
